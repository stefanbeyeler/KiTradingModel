...
from .tcn_training_service import tcn_training_service, TrainingConfig
...
class AutoTrainingConfig:
    ...
    min_symbols: int = 5  # Minimum symbols required to start training
...
class AutoTrainingScheduler:
    ...
    async def run_training_for_all(self, timeframes: Optional[List[str]] = None) -> Dict:
        ...
        Args:
            timeframes: List of timeframes to train. Defaults to config timeframes.
        ...
        for timeframe in timeframes:
            ...
            try:
                result = await tcn_training_service.train(
                    symbols=symbols,
                    timeframe=timeframe,
                    lookback_days=self.config.lookback_days,
                    config=config
        ...
        self.config.next_run = self._calculate_next_run().isoformat() if self._calculate_next_run() else None
        self._save_config()
